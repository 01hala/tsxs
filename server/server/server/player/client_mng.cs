...
namespace Player
{
    ...
    public class PlayerInfo : IHostingData
    {
        ...
        public static IHostingData Load(BsonDocument data)
        {
            ...
            if (data.Contains("LevelNum"))
            {
                info.info.LevelNum = data.GetValue("LevelNum").AsInt32;
            }
            else
            {
                info.info.LevelNum = 1;
            }
    ...
    public class client_mng
    {
        ...
        public client_mng()
        {
            ...
            Hub.Hub._timer.addticktime(10 * 1000, TickCheckStrength);
        }


        private void TickCheckStrength(long tick)
        {
            Log.Log.trace("do TickCheckStrength!");
            foreach (var a in avatarMgr.Avatars)
            {
                var info = a.get_real_hosting_data<PlayerInfo>();
                if (info.Data.CheckStrength())
                {
                    Log.Log.trace("do TickCheckStrength archive_sync UserData {0}!", info.Data.info.User.UserName);
                    PlayerClientCaller.get_client(a.ClientUUID).archive_sync(a.PlayerInfo());
                }
            }
            Hub.Hub._timer.addticktime(10 * 1000, TickCheckStrength);
        }
